...
<style>
    ...
    * {
    ...
    /* Force equal card heights with flexbox - Gemini's recommendation */
    ...
    /* Ensure containers inside columns have equal height */
    div[data-testid="column"] > div {
        height: 100% !important;
    }
    
    /* Container border styling with flex */
    ...
    if st.session_state.get('onboard_step', 1) == 1:
        ...
        with st.container(border=True):
            ...
            with st.expander("Why this mix?"):
                ...
                st.write(f"If the stock market has a bad year (-20%), your stocks might lose ${potential_stock_loss:,}.")
                st.write(f"But your bonds would likely stay steady or gain slightly (+${potential_bond_impact:,}).")
                st.write(f"Net impact: around ${net_loss:,} loss.")
                st.write("")
                st.write(f"If the market has a great year (+15%), your stocks might gain ${potential_stock_gain:,}.")
                st.write(f"Plus bond gains (+${potential_bond_gain:,}).")
